...
export function AdminLeaguesPage() {
  ...
  const startEdit = (entry: CustomEntry) => {
    ...
    setEditSport(entry.sport || 'football');
  };
              ...
              <div key={league.id} className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <span className="text-lg flex-shrink-0 w-6 text-center" title={sportLabel(league.sport)}>
                      {SPORTS.find(s => s.id === league.sport)?.label.split(' ')[0] || '🏅'}
                    </span>
                    {league.logo_url ? (
                      <img
                        src={league.logo_url}
