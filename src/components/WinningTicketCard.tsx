...
export function WinningTicketCard({ news }: WinningTicketCardProps) {
  ...
  return (
    <div className="ticket-premium ticket-won flex h-full flex-col">
      ...
      {thumb ? (
        <div className="relative z-[1] flex min-h-32 items-center justify-center overflow-hidden bg-[#0f0f0f] p-2">
          ...
          <div className="absolute bottom-2 left-3 right-3 flex items-center gap-1.5 bg-[#0f0f0f]/80 px-2 py-1">
      ...
      {thumb && <div className="relative z-[1] border-t border-[#2e2e2e] px-4 py-2"><p className="line-clamp-1 text-[11px] text-textsec">{news.content}</p></div>}
    </div>
  );
}
