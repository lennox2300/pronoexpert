...
export function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForgotSuccess('Un email de réinitialisation vous a été envoyé. Vérifiez votre boîte de réception.');
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'register') {
        ...
        setSuccess('Compte créé ! Un email de confirmation vous a été envoyé. Cliquez sur le lien pour activer votre compte.');
        setTimeout(() => {
          onClose();
          resetForm();
        }, 4000);
        ...
        <div className="p-8">
          ...
          {forgotMode ? (
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              {forgotSuccess && (
                <div className="text-emerald-400 text-sm bg-emerald-900/20 border border-emerald-800 rounded-lg p-3">{forgotSuccess}</div>
              )}
              <button
                type="submit"
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-100 text-gray-800 font-semibold py-3 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed mb-4"
          >
            <GoogleIcon size={20} />
            {googleLoading
              ? 'Connexion...'
          ...
          <div className="mt-6 text-center space-y-2">
            {mode === 'login' && (
              <p className="text-gray-400 text-sm">
                <button
                  onClick={() => { setForgotMode(true); setError(''); setForgotSuccess(''); }}
