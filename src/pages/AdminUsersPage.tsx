...
import { Check, X, Users, Crown, Trash2, Copy } from 'lucide-react';
...
type VIPRequest = Database['public']['Tables']['vip_requests']['Row'];

const VIP_DURATIONS: { label: string; days: 30 | 90 | 180 | 360 }[] = [
...
export function AdminUsersPage() {
  ...
  const [vipDuration, setVipDuration] = useState<30 | 90 | 180 | 360>(30);
  const [showVIPModal, setShowVIPModal] = useState(false);
  const [selectedUserId, setSelectedUserId] = useState<string | null>(null);
  const [modalVipDuration, setModalVipDuration] = useState<30 | 90 | 180 | 360>(30);
                ...
                {newUserIsVIP && (
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      {VIP_DURATIONS.map(({ label, days }) => (
                        <button
                          key={days}
                          type="button"
                          onClick={() => setVipDuration(days)}
                          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                            vipDuration === days ? 'bg-yellow-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'
                          }`}
                        >
                          {label}
                        </button>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {VIP_DURATIONS.map(({ label, days }) => (
                    <button
                      key={days}
                      type="button"
                      onClick={() => setModalVipDuration(days)}
                      className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
                        modalVipDuration === days ? 'bg-yellow-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'
                      }`}
                    >
                      {label}
                    </button>
            ...
            <div className="bg-gray-900 rounded-xl border border-yellow-700/50 overflow-hidden">
              <div className="divide-y divide-gray-800">
                {vipRequests.map((request) => {
                  ...
                  const email = requestUser?.email || 'Utilisateur inconnu';
                  return (
                    <div key={request.id} className="flex items-center justify-between p-4 hover:bg-gray-800/50 flex-wrap gap-3">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <p className="text-white font-semibold">{email}</p>
                          {requestUser?.email && (
                            <button
                              onClick={() => copyEmail(requestUser.email!)}
                              className="flex items-center gap-1 px-2 py-0.5 rounded text-[10px] bg-gray-700 hover:bg-gray-600 text-gray-300 transition-colors"
                              title="Copier l'email"
                            >
                              <Copy size={10} />
                              {copiedEmail === requestUser.email ? 'Copié !' : 'Copier'}
                            </button>
                          )}
                        </div>
                        <p className="text-gray-400 text-xs">
                          Demandé le {request.created_at ? new Date(request.created_at).toLocaleDateString('fr-FR') : '--'} à {request.created_at ? new Date(request.created_at).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) : '--'}
                        </p>
                      </div>
