...
import { Newspaper, Plus, X, Eye, EyeOff, Crown, CreditCard as Edit, Play, Volume2, ChevronDown, ChevronUp, ThumbsUp, Share2, ImagePlus, ChevronLeft, ChevronRight, Pause, Lock, Zap, RotateCcw, Check, TrendingUp, TrendingDown, Wallet, Target } from 'lucide-react';
...
function ArticleCard({ news, isExpanded, onToggle, isAdmin, onTogglePublic, onEdit, onMarkWon, onMarkLost, onDelete, onLike, onShare, localLiked, copied }: ArticleCardProps) {
  ...
  const shareUrl = 'https://pronoexpert.netlify.app/';
...
interface NewsFormProps {
  ...
  odds: string; setOdds: (v: string) => void;
  uploading: boolean;
...
function NewsForm({ title, setTitle, content, setContent, imageUrl, setImageUrl, mediaUrl, setMediaUrl, isPublic, setIsPublic, category, setCategory, eventTime, setEventTime, galleryImages, setGalleryImages, stake, setStake, odds, setOdds, uploading, onGalleryUpload, onSubmit, onCancel, submitLabel, accentFocus, submitBg }: NewsFormProps) {
  return (
    <form onSubmit={onSubmit} className="space-y-3">
      ...
      {category === 'prediction' && (
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1">Mise (€)</label>
            <input type="number" step="0.01" min="0" value={stake} onChange={(e) => setStake(e.target.value)}
              placeholder="ex: 50"
              className={`w-full px-3 py-2 bg-gray-900 border border-gray-800 rounded text-white text-sm focus:outline-none ${accentFocus}`} />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1">Cote totale</label>
...
export function NewsPage({ onPageChange, categoryFilter = 'all' }: { onPageChange?: (p: string) => void; categoryFilter?: 'all' | 'prediction' | 'infos' }) {
  ...
  const [vipBankroll, setVipBankroll] = useState<{ starting_balance: number; current_balance: number; total_profit: number; total_loss: number; won_count: number; lost_count: number } | null>(null);

  const { profile } = useAuth();

  useEffect(() => { loadNews(); loadForm(); loadHistory(); loadVipBankroll(); }, [profile]);
  ...
  const loadVipBankroll = async () => {
    try {
      const { data: configData, error: configErr } = await supabase
        .from('vip_bankroll_config')
        .select('*')
        .maybeSingle();
      ...
      for (const t of tickets || []) {
        ...
        const oddsVal = parseFloat(t.odds) || 0;
        if (t.status === 'won') {
  ...
  const handleCreateNews = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await supabase.from('news').insert({
        ...
        stake: category === 'prediction' && stake ? parseFloat(stake) : null,
        odds: category === 'prediction' && odds ? parseFloat(odds) : null,
      });
  ...
  const startEditNews = (news: News) => {
    ...
    setStake(news.stake != null ? String(news.stake) : '');
    setOdds(news.odds != null ? String(news.odds) : '');
  ...
  const markStatus = async (id: string, status: 'won' | 'lost') => {
    ...
    await recalcVipBankroll();
    loadNews();
    loadForm();
    loadHistory();
  };

  const recalcVipBankroll = async () => {
    try {
      ...
      if (updated) setVipBankroll(updated);
    } catch (err) {
      console.error('Recalc bankroll error:', err);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-black pt-12 pb-4 px-3">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Form indicator - 16 dots max */}
        <div className="mb-4">
          <div className="mb-2 flex items-center gap-1.5">
            <span className="text-[10px] text-gray-500 uppercase font-semibold whitespace-nowrap">Forme sur les {formDots.length} derniers paris</span>
          </div>
        ...
        {/* VIP Bankroll */}
        {vipBankroll && (
          <div className="mb-6 bg-gradient-to-br from-gray-900 via-gray-950 to-black rounded-2xl border border-emerald-700/40 overflow-hidden shadow-xl">
            <div className="flex items-center justify-between px-4 sm:px-5 py-3 bg-gradient-to-r from-emerald-900/30 to-transparent border-b border-emerald-800/30">
              <div className="flex items-center gap-2">
                <Wallet className="text-emerald-400" size={18} />
                <h2 className="text-sm sm:text-base font-bold text-white uppercase tracking-wide">Bankroll VIP</h2>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="bg-gray-900/60 rounded-xl p-3 border border-gray-800">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-[10px] text-gray-500 uppercase font-semibold">Taux</span>
                  </div>
                  <p className="text-lg sm:text-xl font-bold text-emerald-400">
                    {vipBankroll.won_count + vipBankroll.lost_count > 0
                      ? ((vipBankroll.won_count / (vipBankroll.won_count + vipBankroll.lost_count)) * 100).toFixed(0) + '%'
                      : '76%'}
                  </p>
                </div>
                {/* Profit / Perte */}
                <div className="bg-gray-900/60 rounded-xl p-3 border border-gray-800">
                  <div className="flex items-center gap-1.5 mb-1">
                    {vipBankroll.total_profit - vipBankroll.total_loss >= 0
                      ? <TrendingUp className="text-emerald-400" size={14} />
                      : <TrendingDown className="text-red-400" size={14} />}
                    <span className="text-[10px] text-gray-500 uppercase font-semibold">Profit</span>
                  </div>
                  <p className={`text-lg sm:text-xl font-bold ${(vipBankroll.total_profit - vipBankroll.total_loss) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    {(vipBankroll.total_profit - vipBankroll.total_loss) >= 0 ? '+' : ''}{(vipBankroll.total_profit - vipBankroll.total_loss).toLocaleString('fr-FR', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} €
                  </p>
                </div>
                {/* Gagnés */}
                <div className="bg-gray-900/60 rounded-xl p-3 border border-gray-800">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-[10px] text-gray-500 uppercase font-semibold">Gagnés</span>
                  </div>
                  <p className="text-lg sm:text-xl font-bold text-emerald-400">{vipBankroll.won_count}</p>
                </div>
                {/* Perdus */}
                <div className="bg-gray-900/60 rounded-xl p-3 border border-gray-800">
                  <div className="flex items-center gap-1.5 mb-1">
                    <span className="w-2.5 h-2.5 rounded-full bg-red-500 flex-shrink-0"></span>
                    <span className="text-[10px] text-gray-500 uppercase font-semibold">Perdus</span>
                  </div>
                  <p className="text-lg sm:text-xl font-bold text-red-400">{vipBankroll.lost_count}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          return (
            <div className="mt-8">
              ...
              {sortedMonthKeys.map((monthKey) => {
                ...
                let monthProfit = 0;
                for (const n of monthNews) {
                  const s = Number(n.stake) || 0;
                  const o = Number(n.odds) || 0;
                  if (n.status === 'won') {
                    monthProfit += s * o - s;
                  ...
                  <div key={monthKey} className="mb-3">
                    ...
                    >
                      <div className="flex items-center space-x-2 min-w-0">
                        ...
                        <span className="text-xs text-gray-400 whitespace-nowrap">({won}G - {lost}P)</span>
                      </div>
                      <div className={`text-sm font-bold px-3 py-1 rounded flex-shrink-0 ${
                        monthProfit >= 0
                          ? 'bg-emerald-600 text-white'
