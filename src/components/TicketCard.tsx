...
function statusColor(result: Match['result']): string {
  ...
  if (result === 'lost') return 'text-texttert';
  return 'text-textsec';
}
...
function MatchRow({ match, isLast, isSimple, stake }: MatchRowProps) {
  ...
  return (
    <div className={!isLast ? 'border-b border-[#2e2e2e] pb-3 mb-3' : ''}>
      <div className="mb-2 flex flex-wrap items-center gap-2 text-[10px] text-textsec">
        <span className="text-xs leading-none">{SPORT_ICONS[match.sport || ''] || '🏅'}</span>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className={`font-mono-nums text-sm font-semibold ${match.result === 'won' ? 'text-wingreen' : match.result === 'lost' ? 'text-texttert' : 'text-textsec'}`}>{Number(match.odds).toFixed(2)}</span>
      </div>
      ...
      {isSimple && (
        <div className="mt-3 flex items-center justify-between border-t border-border pt-2 text-[11px] text-textsec">
          ...
          <span className={match.result === 'won' ? 'text-wingreen' : match.result === 'lost' ? 'text-texttert' : 'text-textsec'}>
...
export function TicketCard({ prediction, isAdmin, onValidate, onTogglePublic }: TicketCardProps) {
  ...
  return (
    <div className={`ticket-premium ${isPending ? '' : isWon ? 'ticket-won' : 'ticket-lost'}`}>
      <div className="relative z-[1] flex items-center justify-between border-b border-[#2e2e2e] px-4 py-3">
      ...
      <div className="relative z-[1] px-4 py-4">
        {prediction.matches.map((match, index) => (
          <div key={match.id} className="ticket-match-row -mx-4 px-4">
            <MatchRow match={match} isLast={index === prediction.matches.length - 1} isSimple={isSimple} stake={stake} />
          </div>
        ))}
      </div>

      {!isSimple && (
        <div className="relative z-[1] mx-4 mb-4 flex items-center justify-center divide-x divide-[#2e2e2e] rounded-lg bg-[#0f0f0f] px-2 py-3">
          <div className="flex flex-1 flex-col items-center gap-1 px-2 text-[10px] text-textsec"><span>Cote totale</span><strong className={`font-mono-nums text-sm font-semibold ${isPending ? 'text-textsec' : isWon ? 'text-wingreen' : 'text-texttert'}`}>{Number(prediction.total_odds).toFixed(2)}</strong></div>
          ...
          <div className="flex flex-1 flex-col items-center gap-1 px-2 text-[10px] text-textsec"><span>Gains</span><strong className={`font-mono-nums text-sm font-semibold ${isPending ? 'text-textsec' : isWon ? 'text-wingreen' : 'text-losedred'}`}>{isPending ? gain.toFixed(2) : isWon ? `+${(gain - stake).toFixed(2)}` : `-${stake.toFixed(2)}`} €</strong></div>
        </div>
      )}

      {isAdmin && isPending && (
        <div className="relative z-[1] flex flex-wrap gap-2 border-t border-[#2e2e2e] px-4 py-3">
