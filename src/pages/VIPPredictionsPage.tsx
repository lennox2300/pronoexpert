    ...
    <div className="mb-3">
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          <ChevronDown className={`text-yellow-500 flex-shrink-0 transition-transform ${open ? '' : '-rotate-90'}`} size={18} />
          <span className="text-sm font-bold text-yellow-500 uppercase truncate">{title}</span>
        </div>
...
export function VIPPredictionsPage({ onPageChange }: { onPageChange?: (p: string) => void }) {
  ...
  const validatedPredictions = validatedHistory;
