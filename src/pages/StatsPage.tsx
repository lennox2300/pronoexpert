...
function FormDots({ results }: { results: ('won' | 'lost')[] }) {
  ...
  while (last6.length < 6) last6.push(null as unknown as 'won' | 'lost');
  return (
    <div className="flex items-center gap-[3px]">
      {last6.map((r, i) => {
        if (!r) return <div key={i} className="w-2.5 h-2.5 rounded-full bg-gray-800 border border-gray-700/60" />;
        return (
          <div
            key={i}
            className={`w-2.5 h-2.5 rounded-full shadow-sm ${r === 'won' ? 'bg-emerald-500 shadow-emerald-900/60' : 'bg-red-500 shadow-red-900/60'}`}
          />
        );
      })}
    </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-1 w-full">
                {s.sport === 'multisport'
                  ? <span className="text-[9px] font-bold text-gray-300 leading-none flex-shrink-0">Multi</span>
...
export function StatsPage() {
  ...
  const totalStake = predictions.reduce((sum, p) => sum + (Number(p.stake) || 0), 0);
    ...
    <div className="min-h-screen bg-black py-4 px-3">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-2 mb-6">
          <div className="bg-gradient-to-br from-gray-900 to-black border border-emerald-700/50 rounded-lg p-2.5 shadow-lg">
            <div className="flex items-center justify-between mb-1">
              <div className="text-[10px] text-gray-400 uppercase tracking-wide">ROI</div>
              <TrendingUp className={roi >= 0 ? "text-emerald-500" : "text-red-500"} size={14} />
            </div>
            <div className={`text-lg font-bold ${roi >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
              {roi >= 0 ? '+' : ''}{roi.toFixed(1)}%
            </div>
          </div>
