...
import { LEAGUES, NBA_TEAMS, FOOTBALL_LEAGUES, EUROLEAGUE_TEAMS } from '../lib/leaguesData';
...
export function AdminPage() {
  ...
  const [customLeagues, setCustomLeagues] = useState<Suggestion[]>([]);
  const [customTeams, setCustomTeams] = useState<Suggestion[]>([]);
  const [customBetTypes, setCustomBetTypes] = useState<Suggestion[]>([]);
  const [allCustomLeagues, setAllCustomLeagues] = useState<Suggestion[]>([]);
  const [allCustomTeams, setAllCustomTeams] = useState<Suggestion[]>([]);

  const loadCustomEntries = useCallback(async () => {
    const [leaguesRes, teamsRes, betTypesRes] = await Promise.all([
      supabase.from('custom_entries').select('name, logo_url, sport').eq('entry_type', 'league').order('name'),
      supabase.from('custom_entries').select('name, logo_url, competition, sport').eq('entry_type', 'team').order('name'),
      supabase.from('custom_entries').select('name').eq('entry_type', 'bet_type').order('name'),
    ]);
    if (leaguesRes.data) {
      setAllCustomLeagues(leaguesRes.data as Suggestion[]);
      setCustomLeagues(leaguesRes.data as Suggestion[]);
    }
    if (teamsRes.data) {
      setAllCustomTeams(teamsRes.data as Suggestion[]);
      setCustomTeams(teamsRes.data as Suggestion[]);
    }
    if (betTypesRes.data) setCustomBetTypes(betTypesRes.data as Suggestion[]);
  }, []);

  const saveCustomEntry = useCallback(async (
    entryType: 'league' | 'team' | 'bet_type',
    name: string,
    logoUrl?: string | null,
    ...
    const { data: existing } = await supabase
      ...
      .maybeSingle();
    if (existing) {
      const updates: { logo_url?: string; sport?: string; competition?: string } = {};
      if (logoUrl) updates.logo_url = logoUrl;
      if (sport) updates.sport = sport;
      if (competition !== undefined) updates.competition = competition || null;
      if (Object.keys(updates).length > 0) {
        await supabase.from('custom_entries').update(updates).eq('id', existing.id);
      }
      return;
    }
    ...
    try {
      if (editingPrediction) {
        ...
        const matchesData = matches.map((match) => ({
          ...
          team1_logo_url: match.team1_logo_url || null,
          team2_logo_url: match.team2_logo_url || null,
          competition_logo_url: match.competition_logo_url || null,
        }));

        const { error: matchError } = await supabase.from('matches').insert(matchesData);

        if (matchError) throw matchError;
      } else if (type === 'simple' && matches.length > 1) {
      ...
      for (const match of matches) {
        if (match.leagueId === 'other' && match.competition.trim()) {
          await saveCustomEntry('league', match.competition, match.competition_logo_url || null, null, match.sport);
        }
        if (!LEAGUES.find(l => l.id === match.leagueId) && !match.leagueId.startsWith('custom:')) {
          if (match.team1.trim()) await saveCustomEntry('team', match.team1, match.team1_logo_url || null, match.competition || null, match.sport);
          if (match.team2.trim()) await saveCustomEntry('team', match.team2, match.team2_logo_url || null, match.competition || null, match.sport);
        } else if (match.leagueId === 'other' || match.leagueId.startsWith('custom:')) {
          if (match.team1.trim()) await saveCustomEntry('team', match.team1, match.team1_logo_url || null, match.competition || null, match.sport);
          if (match.team2.trim()) await saveCustomEntry('team', match.team2, match.team2_logo_url || null, match.competition || null, match.sport);
        }
  ...
  const handleEditPrediction = (prediction: PredictionWithMatches) => {
    ...
    const formattedMatches: MatchInput[] = prediction.matches.map((match) => {
      ...
      const knownLeague = LEAGUES.find(l =>
        l.name.toLowerCase() === competition.toLowerCase() ||
        l.id === competition.toLowerCase()
      );
      const customLeague = allCustomLeagues.find(c =>
        c.name.toLowerCase() === competition.toLowerCase()
      );
      const leagueId = knownLeague?.id || (customLeague ? `custom:${customLeague.name}` : '');
      return {
        ...
        competition_logo_url: match.competition_logo_url || '',
      };
    });
        ...
        {showCreateForm && (
          <div className="bg-gray-900 rounded-xl p-6 mb-8 border border-gray-800 shadow-lg">
            ...
            <form onSubmit={handleCreatePrediction} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                      ...
                      >
                        ...
                        <option value="boxing">{SPORT_ICONS.boxing} Boxe</option>
                        <option value="mma">{SPORT_ICONS.mma} MMA / UFC</option>
                        <option value="golf">{SPORT_ICONS.golf} Golf</option>
                        <option value="volleyball">{SPORT_ICONS.volleyball} Volley</option>
                        <option value="handball">{SPORT_ICONS.handball} Handball</option>
                        <option value="baseball">{SPORT_ICONS.baseball} Baseball</option>
                        <option value="cycling">{SPORT_ICONS.cycling} Cyclisme</option>
                      </select>
                    </div>

                    <div>
                      <label className="block text-sm text-gray-400 mb-1">Ligue / Competition</label>
                      <select
                        value={match.leagueId}
                        onChange={(e) => {
                          const val = e.target.value;
                          const league = val === 'other' ? null : LEAGUES.find(l => l.id === val);
                          const customLeague = val.startsWith('custom:') ? allCustomLeagues.find(c => c.name === val.slice(7)) : null;
                          const newMatches = [...matches];
                          newMatches[index] = {
                            ...newMatches[index],
                            leagueId: val,
                            competition: league?.name || (customLeague?.name || ''),
                            team1: '',
                            team2: '',
                            team1_logo_url: '',
                            team2_logo_url: '',
                            competition_logo_url: customLeague?.logo_url || '',
                          };
                      ...
                      >
                        ...
                        {LEAGUES.filter(l => l.id !== 'other').map(l => (
                          <option key={l.id} value={l.id}>
                            {l.flag} {l.name}
                          </option>
                        ))}
                        {allCustomLeagues
                          .filter(l => !l.sport || l.sport === match.sport)
                          .map((l, i) => (
                            <option key={`custom-${i}`} value={`custom:${l.name}`}>
                              {l.logo_url ? '🏆' : '📋'} {l.name}
                            </option>
                          ))}
                        <option value="other">➕ Autre (nouvelle ligue)</option>
                      </select>
                      {match.leagueId === 'other' && (
                        <>
                          <AutocompleteInput
                            value={match.competition}
                            onChange={(v) => updateMatch(index, 'competition', v)}
                            onSelect={(s) => {
                              setMatches(prev => {
                                const nm = [...prev];
                                nm[index] = { ...nm[index], competition: s.name, competition_logo_url: s.logo_url || '' };
                                return nm;
                              });
                            }}
                            suggestions={allCustomLeagues.filter(l => !l.sport || l.sport === match.sport)}
                            placeholder="Nom de la competition"
                            className="w-full mt-1 px-3 py-1.5 bg-gray-900 border border-gray-700 rounded text-white text-xs focus:outline-none focus:ring-1 focus:ring-green-500"
                            required
                          />
                          <input
                            type="url"
                            value={match.competition_logo_url}
                            onChange={(e) => updateMatch(index, 'competition_logo_url', e.target.value)}
                            placeholder="URL logo competition (optionnel)"
                            className="w-full mt-1 px-3 py-1.5 bg-gray-900 border border-gray-700 rounded text-gray-300 text-xs focus:outline-none focus:ring-1 focus:ring-green-500"
                          />
                        </>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      {match.leagueId === 'nba' ? (
                        ...
                        >
                          ...
                          {NBA_TEAMS.map(t => (
                            <option key={t.nbaCode} value={t.name}>{t.shortName} - {t.name}</option>
                          ))}
                        </select>
                      ) : match.leagueId === 'euroleague' ? (
                        <select
                          value={match.team1}
                          onChange={(e) => updateMatch(index, 'team1', e.target.value)}
                          className="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded text-white text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                        ...
                        >
                          <option value="">-- Equipe 1 --</option>
                          {EUROLEAGUE_TEAMS.map(t => (
                            <option key={t.file} value={t.name}>{t.shortName}</option>
                          ))}
                        </select>
                      ) : FOOTBALL_LEAGUES[match.leagueId] ? (
                        <select
                          value={match.team1}
                          onChange={(e) => updateMatch(index, 'team1', e.target.value)}
                          className="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded text-white text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                        ...
                        >
                          <option value="">-- Equipe 1 --</option>
                          {FOOTBALL_LEAGUES[match.leagueId].teams.map(t => (
                            <option key={t.file} value={t.file}>{t.shortName}</option>
                          ))}
                        </select>
                      ) : (
                        <AutocompleteInput
                          value={match.team1}
                          onChange={(v) => updateMatch(index, 'team1', v)}
                          onSelect={(s) => {
                            setMatches(prev => {
                              const nm = [...prev];
                              nm[index] = { ...nm[index], team1: s.name, team1_logo_url: s.logo_url || '' };
                              return nm;
                            });
                          }}
                          suggestions={allCustomTeams.filter(t =>
                            (!t.sport || t.sport === match.sport) &&
                            t.competition === match.competition
                          )}
                          placeholder="Nom equipe / joueur"
                          className="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded text-white text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                          required
                        />
                      )}
                      {(match.leagueId === 'other' || match.leagueId.startsWith('custom:')) && (
                        <input
                          type="url"
                          value={match.team1_logo_url}
                          onChange={(e) => updateMatch(index, 'team1_logo_url', e.target.value)}
                          placeholder="URL logo equipe 1 (optionnel)"
                          className="w-full mt-1 px-3 py-1.5 bg-gray-900 border border-gray-700 rounded text-gray-300 text-xs focus:outline-none focus:ring-1 focus:ring-green-500"
                        />
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      {match.leagueId === 'nba' ? (
                        ...
                        <AutocompleteInput
                          value={match.team2}
                          onChange={(v) => updateMatch(index, 'team2', v)}
                          onSelect={(s) => {
                            setMatches(prev => {
                              const nm = [...prev];
                              nm[index] = { ...nm[index], team2: s.name, team2_logo_url: s.logo_url || '' };
                              return nm;
                            });
                          }}
                          suggestions={allCustomTeams.filter(t =>
                      ...
                      {(match.leagueId === 'other' || match.leagueId.startsWith('custom:')) && (
                        <input
                          ...
                          onChange={(e) => updateMatch(index, 'team2_logo_url', e.target.value)}
                          placeholder="URL logo equipe 2 (optionnel)"
                          className="w-full mt-1 px-3 py-1.5 bg-gray-900 border border-gray-700 rounded text-gray-300 text-xs focus:outline-none focus:ring-1 focus:ring-green-500"
                        />
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <AutocompleteInput
                        ...
                        onChange={(v) => updateMatch(index, 'bet_type', v)}
                        suggestions={customBetTypes}
              ...
              >
                {type === 'simple' && !editingPrediction
                  ? '+ Ajouter un match simple'
                  : '+ Ajouter un match'}
              </button>

              {type === 'combined' || editingPrediction ? (
                ...
                <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
                  <div className="text-sm text-gray-400 mb-1">
                    {matches.length} match{matches.length > 1 ? 's' : ''} simple{matches.length > 1 ? 's' : ''} - {matches.length} ticket{matches.length > 1 ? 's' : ''} seront crees
                  </div>
                  <div className="text-sm text-gray-500">
                    Mise par ticket: {stake || '0'} EUR
                  </div>
                </div>
              )}
